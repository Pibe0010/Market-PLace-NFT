...
const projectId = '2SpVMALpl8HWygbHpNxl7CsDgFI';
const projectSecret = '5b0b15ffd951c785f7b2e0d3f92f0d0e';
...
const Create = ({ marketplace, nft }) => {
    ...
    const uploadToIPFS = async (event) => {
        ...
        if (typeof file !== 'undefined') {
            try {
                ...
                console.log(result);
                setImage(`https://nftdapp.infura-ipfs.io/ipfs/${result.path}`);
            } catch (error) {
    ...
    const mintThenList = async (result) => {
        const uri = `https://nftdapp.infura-ipfs.io/ipfs/${result.path}`;
